...
if uploaded_file is not None:
    with st.spinner('Lendo e processando os dados... Isso pode levar alguns segundos.'):
        ...
        # Tratamento das colunas de NS (Nível de Serviço)
        ...
        # Tratamento da coluna CMU caso venha como texto (com R$ ou vírgulas)
        if 'CMU' in df.columns and df['CMU'].dtype == object:
            df['CMU'] = df['CMU'].astype(str).str.replace('R$', '', regex=False).str.replace(' ', '', regex=False).str.replace(',', '.').astype(float)
            
        # Tratamento do Prazo Prometido por precaução
        if 'Prazo Prometido' in df.columns and df['Prazo Prometido'].dtype == object:
            df['Prazo Prometido'] = df['Prazo Prometido'].astype(str).str.replace(',', '.').astype(float)
